...
class DTLearner(object):
    ...
    def add_evidence(self, dataX, dataY):
        ...
        if self.verbose:
            print("DT tree size:", self.tree.shape)
    ...
    def build_tree(self, dataX, dataY):
        ...
        best_feature_index = self.optimal_feature(dataX, dataY)
        split_val = np.median(dataX[:, best_feature_index])

        left_mask = dataX[:, best_feature_index] <= split_val
        ...
        root = np.array([best_feature_index, split_val, 1, left_tree.shape[0] + 1 if left_tree.ndim > 1 else 2])
    ...
    def optimal_feature(self, dataX, dataY):
        correlations = np.array(
            [np.corrcoef(dataX[:, i], dataY)[0, 1] if np.std(dataX[:, i]) > 0 else 0 for i in range(dataX.shape[1])])
